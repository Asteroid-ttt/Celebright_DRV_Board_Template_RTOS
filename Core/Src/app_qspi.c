/**
 * @file app_qspi.c
 * @brief QSPI Flash 服务层 —— W25Q256 + LittleFS 文件系统
 *
 * 提供 flash mount/ls/cat/info/format 等高层接口，
 * 供 CLI 命令和应用程序调用。
 */
#include "app_qspi.h"
...
bool AppQSPI_Mount(void)
{
    ...
    if (!W25Q256_IsPresent()) return false;
...
{
    return W25Q256_GetName();
}

uint32_t AppQSPI_ReadID(void)
{
    return W25Q256_ReadID();
}
...
size_t AppQSPI_BuildInfo(char *buf, size_t buf_len)
{
    ...
    if (!g_lfs_mounted)
    {
        (void)snprintf(buf, buf_len,
            ...
            W25Q256_GetName(),
            (unsigned long)(W25Q256_GetCapacity() / 1024UL / 1024UL));
    }
    else
    {
        lfs_ssize_t used_blocks = lfs_fs_size(&g_lfs);
        uint32_t total_kb = W25Q256_GetCapacity() / 1024UL;
