 ...
 *
 * 使用 Flash 前 16 MB 作为 LittleFS 分区。
 */
#include "lfs_port.h"
#include "w25q256.h"
...
#define LFS_BLOCK_COUNT 4096U  /* 16 MB data area = 4096 * 4 KB */
                    ...
                    lfs_off_t off, void *buf, lfs_size_t size)
{
    ...
    if (W25Q256_Read(addr, (uint8_t *)buf, size))
        return 0;
    return LFS_ERR_IO;
}
                    ...
                    lfs_off_t off, const void *buf, lfs_size_t size)
{
    (void)c;
    uint32_t addr = (uint32_t)(block * LFS_BLOCK_SIZE + off);
    if (W25Q256_Write(addr, (const uint8_t *)buf, size))
        return 0;
    return LFS_ERR_IO;
}
...
{
    (void)c;
    uint32_t addr = (uint32_t)(block * LFS_BLOCK_SIZE);
    if (W25Q256_EraseSector(addr))
        return 0;
    return LFS_ERR_IO;
}
