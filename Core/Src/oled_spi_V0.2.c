...
//		OLED_CS_Clr();
		...
		while (HAL_SPI_GetState(&hspi2) != HAL_SPI_STATE_READY) {	// 等待 SPI 不再忙碌
		}
...
void OLED_Refresh(void)
{
	...
	for(i=0;i<8;i++)
	{
		...
		*/
		...
		uint8_t data[] = {0x78, 0x78, 0x40}; // 要发送的固定数据
		HAL_SPI_Transmit(&hspi2, data, sizeof(data), HAL_MAX_DELAY);  // 发送数据数组
		while (HAL_SPI_GetState(&hspi2) != HAL_SPI_STATE_READY) {	// 等待 SPI 不再忙碌
		}
		for (n = 0; n < 128; n++) {
				HAL_SPI_Transmit(&hspi2, &OLED_GRAM[n][i], 1, HAL_MAX_DELAY);  // 发送每个字节
				while (HAL_SPI_GetState(&hspi2) != HAL_SPI_STATE_READY) {	// 等待 SPI 不再忙碌
				}
		}
		// I2C_Stop();
