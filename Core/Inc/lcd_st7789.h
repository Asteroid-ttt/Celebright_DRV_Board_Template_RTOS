...
/* 2D coordinate */
typedef struct COORDINATE { int x; int y; } TypeXY;
#define point TypeXY

/* Rotation struct */
typedef struct ROATE { TypeXY center; float angle; int direct; } TypeRoate;
...
struct _LCD_CTX { uint32_t Color; uint32_t BackColor; uint8_t ShowNum_Mode; uint8_t Direction; uint16_t Width; uint16_t Height; uint8_t X_Offset; uint8_t Y_Offset; };
extern struct _LCD_CTX LCD;
extern unsigned char ScreenBuffer[8][240];
extern TypeRoate _RoateValue;
